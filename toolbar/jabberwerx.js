...
(function($) {
    ...
    $.extend($.jStore, {
        ...
        ready: function(callback) {
            if ($.jStore.isReady)
                callback.apply($.jStore, [$.jStore.CurrentEngine]);
            else
                $.jStore.delegate.bind('jStore-ready', callback);
        },
        ...
        flashReady: function(callback) {
            if ($.jStore.isFlashReady)
                ...
                $.jStore.delegate.bind('flash-ready', callback);
        },
        use: function(engine, project, identifier) {
            ...
            if (!e)
                throw 'JSTORE_ENGINE_UNDEFINED';
            e = new e(project,name);
            if ($.jStore.Instances[name])
                ...
                if (!e.autoload)
                    ...
                    e.included(function() {
                        if (this.isAvailable()) {
                            ...
                            $.jStore.delegate.trigger('jStore-failure', this);
                    }).include();
                }
            }
        },
...
(function($) {
    this.StorageEngine = Class.extend({
            ...
            $.each(this.includes, function() {
                $.ajax({
                    ...
                    success: function() {
                        count++;
                        if (count == total)
                            self.delegate.trigger('engine-included');
                    }
                })
            });
        },
        ...
        interruptAccess: function() {
            if (!this.isReady)
                throw 'JSTORE_ENGINE_NOT_READY';
        },
        ready: function(callback) {
            if (this.isReady)
                callback.apply(this);
            else
                this.delegate.bind('engine-ready', callback);
            return this;
        },
        included: function(callback) {
            if (this.hasIncluded)
                ...
                this.delegate.bind('engine-included', callback);
            return this;
        },
        get: function(key) {
    ...
    this.jStoreFlash = StorageEngine.extend({
        ...
        flashReady: function(e) {
            ...
            if (iFrame.Document && $.isFunction(iFrame.Document['jStoreFlash'].f_get_cookie))
                ...
                if ($.isFunction($('object', $(doc))[0].f_get_cookie))
                    this.db = $('object', $(doc))[0];
                else if ($.isFunction($('embed', $(doc))[0].f_get_cookie))
                    this.db = $('embed', $(doc))[0];
            }
            if (this.db)
                this.delegate.trigger('engine-ready');
...
(function($) {
    ...
    this.jStoreHtml5 = StorageEngine.extend({
        ...
        connect: function() {
            ...
            if (!db)
                throw 'JSTORE_ENGINE_HTML5_NODB';
                ...
                if (jabberwerx.util.isArray(root)) {
                    ...
                    for (var p in root) {
                        try {
                            ...
                            jabberwerx.util.debug.log('Exception throw while searching for reachable GUIDs: ' + ' Property: ' + p + ' Exception: ' + e.message);
                        }
                    }
                }
            }
...
;(function(jabberwerx) {
    ...
    jabberwerx.util.crypto.b64Encode = function(input) {
        ...
        for (var idx = 0; idx < input.length; idx += 3) {
            ...
            output += table.charAt((data >>> 18) & 0x003f) + table.charAt((data >>> 12) & 0x003f);
            ...
            for (var idx = 0; !selected && idx < this.mechanisms.length; idx++) {
                ...
                for (var jdx = 0; !selected && jdx < mechs.length; jdx++) {
                    ...
                    try {
                        ...
                        jabberwerx.util.debug.log("could not create SASLMechanism for " + candidate.mechanismName + ": " + ex);
                        selected = null;
                    }
                }
            }
...
;(function(jabberwerx) {
    ...
    var _lookupCache = function(val) {
        ...
        val = (resSep != -1) ? val.substring(0, resSep).toLowerCase() + "/" + val.substring(resSep + 1) : val.toLowerCase();
        jid = _jidCache[val];
        if (jid) {
            return jid;
        }
    ...
    jabberwerx.JID = jabberwerx.JWModel.extend({
        init: function(arg) {
            ...
            this._full = "" + (arg.node ? arg.node + "@" : "") + arg.domain + (arg.resource ? "/" + arg.resource : "");
...
;(function(jabberwerx) {
    ...
    jabberwerx.Entity = jabberwerx.JWModel.extend({
        ...
        __toStringValue: function() {
            return "entity<" + this.getClassName() + ">[" + this._mapKey + "]: " + this.getDisplayName();
        },
        ...
        _insertPresence: function(presence) {
            var ipos;
            for (ipos = 0; (ipos < this._presenceList.length) && (presence.compareTo(this._presenceList[ipos]) < 0); ++ipos)
                ;
...
;(function(jabberwerx) {
    jabberwerx.Stream = jabberwerx.JWModel.extend({
        ...
        _storeConnectionInfo: function(protocol, hostPort, resource) {
            this._boshProps.httpBindingURL = protocol + "//" + hostPort + "/" + resource;
        ...
        _handleXHR: function(xhr, status) {
            ...
            var failFn = function(err, resend) {
                ...
                if (boshProps.networkAttempts++ < 3) {
                    jabberwerx.util.debug.log("network timeout retry " + boshProps.networkAttempts);
                    if (resend) {
...
;(function(jabberwerx) {
    ...
    jabberwerx.Client = jabberwerx.JWModel.extend({
        ...
        _handleEntityTime: function(evt) {
            ...
            h = tzo / 60;
            m = tzo % 60;
            tzo = (tzo > 0 ? '-' : '+') + (h < 10 ? '0' + h : h) + ':' + (m < 10 ? '0' + m : m);
            var iq = evt.data;
            iq = iq.reply();
            var query = new jabberwerx.NodeBuilder(iq.getQuery());
...
;(function(jabberwerx) {
    ...
    jabberwerx.MUCRoom = jabberwerx.Entity.extend({
        ...
        applyConfig: function(configForm, configCallback) {
            ...
            var that = this;
            var iqcb = cancel ? function(stanza) {}
            : function(stanza) {
                if (configCallback) {
    ...
    jabberwerx.ChatSession = jabberwerx.JWModel.extend({
        ...
        _setChatStateProperty: function(state) {
            ...
            if (jabberwerx.$.inArray(state, ['active', 'composing', 'paused', 'inactive', 'gone']) >= 0) {
                ...
                var err = new jabberwerx.ChatSession.StateNotSupportedError("The chat state '" + state + "' is not supported. Should be one of 'active', 'composing'," + "'paused', 'inactive' or 'gone'");
                throw err;
            }
            return retVal;
        },
        ...
        submit: function(fieldsAndValues) {
            ...
            for (var i = 0; i < this.fields.length; i++) {
                ...
                if (idx == -1 && (this.fields[i].getValues().length || this.fields[i].getRequired())) {
                    if (this.fields[i].getType() == "fixed" || this.fields[i].getType() == "hidden")
                        continue;
...
;(function(jabberwerx) {
    jabberwerx.CapabilitiesController = jabberwerx.Controller.extend({
        ...
        _getIdentity: function() {
            return (this.identity.category ? this.identity.category : '') + '/' + (this.identity.type ? this.identity.type : '') + '//' + (this.identity.name ? this.identity.name : '');
        },
        ...
        identity: {
            ...
            type: 'pc',
            toString: function() {
                return (this.category ? this.category : '') + '/' + (this.type ? this.type : '') + '//' + (this.name ? this.name : '');
            }
        },
