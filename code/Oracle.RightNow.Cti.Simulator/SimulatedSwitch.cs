...
namespace Oracle.RightNow.Cti.Simulator {
    ...
    public class SimulatedSwitch {
        ...
        public static void Run(IGlobalContext rightNowGlobalContext) {
            var catalog = new AssemblyCatalog(typeof(SimulatedSwitch).Assembly);
            ...
            _container.GetExport<SimulatedSwitch>().Value._messagingHandler.Initialize();
        ...
        internal void AssignQueuedInteraction(SwitchInteraction interaction) {
            ...
            if (interaction.Type == InteractionType.Call)
                predicate = d => d.Agent.State == SwitchAgentState.Available && !d.Agent.Interactions.Any(i => i.Type == InteractionType.Call);
            else
